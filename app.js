...
function parseCzechDate(s){const m=String(s||'').trim().match(/^(\d{1,2})\.(\d{1,2})\.(\d{4})$/);if(!m)return null;const d=Number(m[1]),mo=Number(m[2]),y=Number(m[3]);const dt=new Date(y,mo-1,d);if(dt.getFullYear()!==y||dt.getMonth()!==mo-1||dt.getDate()!==d)return null;return `${y}-${String(mo).padStart(2,'0')}-${String(d).padStart(2,'0')}`;}
function setService(id,kind){const v=vehicles.find(x=>x.id===id);if(!v)return;const labels={stk:'Datum platnosti STK (DD.MM.RRRR)',oil:'Datum příští výměny oleje (DD.MM.RRRR)',reminder:'Datum připomínky (DD.MM.RRRR)'};const keys={stk:'stkDate',oil:'oilDate',reminder:'reminderDate'};const current=v[keys[kind]]?formatDate(v[keys[kind]]):'';const val=prompt(labels[kind],current);if(val===null)return;if(!val.trim()){v[keys[kind]]='';save();render();return;}const parsed=parseCzechDate(val);if(!parsed){alert('Zadej datum ve formátu DD.MM.RRRR, například 28.08.2027.');return;}v[keys[kind]]=parsed;save();render();}
